...
psFileName_t inFileName, outFileName;
psFileName_t printFileName, tmpFileName;
byte columns;
...
word segSizes[256];
byte segFlags[256];
pointer inRecordP;	// modified to support none le systems
pointer erecP;
pointer inP;
word recNum;
word recLen;
...
char crlf[2] = { '\r', '\n' };
char aRecordType[] = " RECORD TYPE XXH, RECORD NUMBER *****\r\n";
...
void FatalErr(byte errCode)
{
    FlushPrintBuf();
    ConStrOut(" ", 1);
    ConStrOut(inFileName.str, inFileName.len);
    ConStrOut(",", 1);
    Errmsg(errCode);
    BinAsc(inRecordP[RECORD_rectyp], 16, '0', &aRecordType[13], 2);
    if (recNum > 0)
...
void ChkRead(word cnt)
{
    ...
    if ((bcnt = (word)(eiBufP - iBufP)) < cnt)	/* data alReady in buffer */
    {
        ...
        inBlk += (inByt + (word)(iBufP - sibufP)) / 128;
        inByt = (inByt + (word)(iBufP - sibufP)) % 128;
        if ((bcnt = bcnt + actRead) < cnt)
            ErrChkReport(ERR204, inFileName.str, TRUE);  /* Premature() EOF */
...
void GetRecord() {
    ...
    if ((bcnt = (word)(eiBufP - iBufP)) >= 4) {
        inRecordP = iBufP;
        rlen      = getWord(inRecordP + 1);
        erecP     = iBufP + rlen + 2;
    } else {
        inRecordP = nullRec; // will be updated below
        rlen      = 0;
    ...
    if (erecP == NULL || erecP >= eiBufP) /* test modified to handle chnage above*/
    {
        if (rlen <= 1025) {
            ChkRead(bcnt + 1);
            inRecordP = iBufP;
            if ((erecP = iBufP + rlen + 2) >= eiBufP)
                ErrChkReport(ERR204, inFileName.str, true); /* premature EOF */
        }
    }

    recLen = rlen;
    inP    = inRecordP + RECORD_record;

    iBufP  = erecP + 1; /* update for next record */
    recNum++;
    if (inRecordP[RECORD_rectyp] > R_COMDEF || (inRecordP[RECORD_rectyp] & 1)) /* > 0x2e || odd */
        IllegalReloc();
    if (inRecordP[RECORD_rectyp] == R_MODDAT)
        return;
    if (recLen > 1025)
        FatalErr(ERR211); /* Record() too long */

    inCRC = 0; // moved to calculate and update iBufP
    for (inbP = inRecordP; inbP <= erecP; inbP++)
        inCRC += *inbP;
...
void SeekPagingFile(byte para)
{
    ...
    ErrChkReport(statusIO, tmpFileName.str, true);
}
...
void AnotherPage(byte page)
{
    ...
    if (! havePagingFile)	/* have to page, create file if ! done */
    {
        ...
        Open(&tmpfd, tmpFileName.str, 3, false, &statusIO);
        ErrChkReport(statusIO, tmpFileName.str, true);
    }
...
pointer AddrInCache(word addr)
{
    ...
    if (pt->pageIdx == 0xFE)	/* on disk */
    {
        ...
        if (actRead != 256)			/* if problems & ~ :BB: out file then Error() */
            if (outRealFile)
                ErrChkReport(ERR204, tmpFileName.str, true);    /* premature EOF */
...
static void Alloc(word cnt)
{
    ...
    // to avoid sequence point issues, factor out the botHeap -= cnt
    botHeap -= cnt;
    if (High(botHeap - baseMemImage + cnt) != High(botHeap - baseMemImage))
    {
        ...
        if ((pageCacheSize = High((word)(botHeap - baseMemImage)) - 1) == 0xff)	/* check we haven't eliminated all cache */
            ErrChkReport(ERR210, inFileName.str, true);	/* Insufficient() memory */
    }
...
void AddSegFrag(byte flags, byte seg, word start, word len)
{
    ...
    while ((pointer)curSegFragP != topDataFrags) {
        ...
        if (curSegFragP->start > start)
            ...
            {
                curSegFragP->len += len;	/* merge the lengths */
