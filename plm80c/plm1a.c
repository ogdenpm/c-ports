...
// clang-format on
uint8_t builtinsMap[] = { I_CARRY, I_DEC,    I_DOUBLE, I_HIGH,     I_INPUT, I_LAST, I_LENGTH, I_LOW,
                       I_MOVE,  I_OUTPUT, I_PARITY, I_ROL,      I_ROR,   I_SCL,  I_SCR,    I_SHL,
                       I_SHR,   I_SIGN,   I_SIZE,   I_STACKPTR, I_TIME,  I_ZERO };

uint16_t parseSP;
uint16_t parseStack[100];
uint16_t operatorSP;
uint16_t operatorStack[50];
uint16_t operandSP;
...
uint16_t Wr2Item(uint8_t type, void *buf, uint16_t len) {
    if (!hasErrors || (T2_STMTCNT <= linfo.type && linfo.type <= T2_ERROR)) {
        Wr2Byte(type); // split to avoid potential uint16_t alignment
        if (len)
            Wr2Buf(buf, len);
    }
    return ++stmtT2Cnt;
...
*/
static uint16_t Wr2SyncedItem(uint8_t type, void *buf, uint16_t len) {
...
uint16_t CvtToRel(uint16_t nodeIdx) {
...
*/
void MapLToT2(void) {
    uint8_t arglen  = tx1Lengths[tx1Item.type];    // length of T1 args
...
void Wr2TokError(uint16_t errCode) {
    if (info) {
        uint16_t args[2] = { errCode, ToIdx(info) };
        hasErrors    = true;
