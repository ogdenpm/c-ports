...
void Trac(uint8_t t) {
    ...
    while ((t = getche()) != ' ') {
        ...
        case 'S':
            for (i = 0; i <= symTop; i++)
                printf(":%02X:%02X:%04X:%02X:%04X:%04X", i, stb[i], sDispl[i], sLink[i], sAtr[i],
                       sName[i]);
            break;
        case 'R':
...
void SRAddr(uint8_t j, uint8_t b,
            uint16_t d) { /* set address fields of entry j in reltab to (b, d) */
    SRBase(j, b);
    rdispl[j] = d;
}
...
void Emit3A(uint8_t op, uint8_t b, uint16_t d,
            uint16_t e) { /* emit a 3-byte relocatable instruction with address (b,d,e) */
    Emit1(op);
    EmitA(b, d, e);
}
...
void Reduce(uint16_t pn) {
    ...
    case 34: /*** <IF.CLAUSE> ::= IF <COMPOUND.CONDITION> THEN */
        ll = l2;
        break;               /* return pointer to f chain in reltab */
    ...
    case 43:
        xx = x4;
        break; /*** <SIMPLE.CONDITION> ::= ( <STMT.LIST> ) <CONDIITON> */
               /* return a numeric code representing the condition */

    case 44:
        Xmit1();
        break; /*** <SIMPLE.CONDITION> ::= <CONDITION> */
    case 45:
        xx = 0xc0;
        break; /*** <CONDITION> ::= NOT ZERO */
    case 46:
        xx = 0xc8;
        break; /*** <CONDITION> ::= ZERO */
    case 47:
        xx = 0xd0;
        break; /*** <CONDITION> ::= NOT CY */
    case 48:
        xx = 0xd8;
        break; /*** <CONDITION> ::= CY */
    case 49:
        xx = 0xe0;
        break; /*** <CONDITION> ::= PY ODD */
    case 50:
        xx = 0xe8;
        break; /*** <CONDITION> ::= PY EVEN */
    case 51:
        xx = 0xf0;
        break; /*** <CONDITION> ::= PLUS */
    case 52:
        xx = 0xf8;
        break; /*** <CONDITION> ::= MINUS */
    ...
    case 56: /*** <DATA.LIST> ::= <DATA.HEAD> <CONSTANT> ) */
        ...
        Chc(CA);
        break;              /* emit the constant into ca */
    ...
    case 59:   /*** <STORAGE.DECLARATION> ::= <IDENT.SPECIFICATION> BYTE */
        Sdb(1);
        break; /* return pointer to indentifier chain in symtab */
    case 60:
        Sdb(h2);
        break; /*** <STORAGE.DECLARATION> ::= <BOUND.HEAD> <NUMBER> ) BYTE */
    case 61:   /*** <STORAGE.DECLARATION> ::= <STORAGE.DECLARATION> <INITIAL.LIST> */
        Xmit1();
        break; /* <INITIAL.LIST> already stored in ida */
    case 62:
        xx = KLABEL;
        break; /*** <TYPE> :: = LABEL */
    case 63:
    ...
    case 66:
        Idl();
        break;          /*** <IDENT.SPECIFICATION> ::= <IDENT.LIST> <IDENTIFIER> ) */
    case 67:            /*** <IDENT.LIST> ::= ( */
                        /* return pointer to identifier chain in symtab */
        ll = 0;         // pointer
    ...
    case 68:
        Idl();
        break; /*** <IDENT.LIST> ::= <IDENT.LIST> <IDENTIFIER> , */
    case 69:   /*** <BOUND.HEAD> ::= <IDENT.SPECIFICATION> */
        Xmit1();
        break; /* return pointer to identifier chain in symtab */
    case 70:   /*** <INITIAL.LIST> ::= <INITIAL.HEAD> <CONSTANT> ) */
        Chc(IDA);
        break; /* emit the constant into ida */
    ...
    case 73: /*** <GROUP.HEAD> ::= <DO> */
        ZeroXX();
        break; /* return xx indicating type of group */
    ...
    case 81: /*** <WHILE> ::= WHILE */
        hh = caNext;
        break;          /* return address of next instruction (compound.condiiton) */
    ...
    case 87: /*** <FORMAL.PARAM.LIST> ::= <FP.HEAD> <IDENTIFIER> ) */
        Fph();
        break; /* return number of formal parameters */
    case 88:   /*** <FP.HEAD> ::= ( */
        ZeroXX();
        break; /* return no. of formal parameters */
    case 89:
        Fph();
        break; /*** <FP.HEAD> ::= <FP.HEAD> <IDENTIFIER> , */
    case 90:
        Emit1(RET);
        break; /*** <RETURN.STMT> ::= RETURN */
    case 91:   /*** <RETURN.STMT> ::= IF <SIMPLE.CONDITION> RETURN */
        Emit1(x2);
        break; /* emit rnz, rz, rnc ... */
    ...
    case 95:
        Aph();
        break; /*** <ACTUAL.PARAM.LIST> ::= <AP.HEAD> <CONSTANT> ) */
    ...
    case 97:
        Aph();
        break; /*** <AP.HEAD> ::= <AP.HEAD> <CONSTANT> , */
    case 98:
        xx = 0xcd;
        break; /*** <CALL> ::= CALL */
    case 99:
        XmitX2();
        break; /*** <CALL> ::= IF <SIMPLE.CONDITION> CALL */
    ...
    case 102:
        Emit1(PCHL);
        break; /*** <GOTO.STMT> ::= <GOTO> M ( HL ) */
    case 103:
        xx = JMP;
        break; /*** <GOTO> ::= GOTO */
    case 104:
        XmitX2();
        break; /*** <GOTO> ::= IF <SIMPLE.CONDITION> GOTO */
    ...
    case 106: /*** <REPEAT> ::= REPEAT */
        hh = caNext;
        break; /* save address of next instruction */
    case 107:
        Emit1(0x76);
        break; /*** <CONTROL.STMT> :: = HALT */
    case 108:
        Emit1(0);
        break; /*** <CONTROL.STMT> :: = NOP */
    case 109:
        Emit1(0xf3);
        break; /*** <CONTROL.STMT> :: = DISABLE */
    case 110:
        Emit1(0xfb);
        break;                       /*** <CONTROL.STMT> :: = ENABLE */
    ...
    case 130: /*** <REG> ::= <A.L> */
        SetReg((uint8_t)h1);
        break; /* a=7, b=0, c=1, d=2, e=3, h=4, l=5 */
    case 131:
        SetReg(TBC);
        break; /*** <REG> ::= BC */
    case 132:
    ...
    case 148:
        xx = 0x2f;
        break; /*** <BINARY.OP> ::= NOT (cma)*/
    case 149:
        xx = 0x27;
        break; /*** <BINARY.OP> ::= DEC (daa)*/
    case 150:
        xx = 0xf;
        break; /*** <BINARY.OP> ::= >> (rrc) */
    case 151:
