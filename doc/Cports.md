# C ports of PL/M 80 & Fortran & CP/M applications
...
MAC uses various controls to select where to read/write files; its concept of disk drives isn't particularly portable, this version allows file names, with paths to be supplied. If an extent is not included in the filename, then the relevant default extent is added.

For device listings MAC expands tabs. To support this a new -t option is provided to allow tabs to be expanded if needed.

The additional MAC controls, including the undocumented $+R (which relocates by 100H) are now supported using a new -c option which is followed by the various MAC controls without the leading $.

Other changes of note are

1) MACLIB allows a pathname, which cannot contain control characters or ';'. Additionally trailing spaces are trimmed. Unlike the CP/M version, the filename is not mapped to uppercase in the listing.
2) The odd behaviour of MAC with the $+1 option has been modified. Now if the listing is to a file, both the pass 1 and pass 2 information is shown. The original would overlay any non flushed part of the pass 1 listing with the pass 2 listing.
3) The internal symbol table space is larger.
4) The opcode mnemonic is now looked up using a perfect hash generated by gperf. The conditional JMP, CALL and RET are also now compressed into internal tokens when stored as part of a macro definition.

```
Usage: mac [options] sourceFile[.asm]
Where options are
...
```
Updated by Mark Ogden 13-Oct-2025
```